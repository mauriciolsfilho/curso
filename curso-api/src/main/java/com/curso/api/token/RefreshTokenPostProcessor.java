...
public class RefreshTokenPostProcessor implements ResponseBodyAdvice<OAuth2AccessToken> {
	...
	@Autowired
	private CursoApiProperty cursoApiProperty;
    @Override
    ...
    public OAuth2AccessToken beforeBodyWrite(OAuth2AccessToken body, MethodParameter returnType, MediaType selectedContentType, Class<? extends HttpMessageConverter<?>> selectedConverterType, ServerHttpRequest request, ServerHttpResponse response) {
        ...
        adicionarRefreshTokenCookie(refresToken, req, resp);
    ...
    private void adicionarRefreshTokenCookie(String refresToken, HttpServletRequest req, HttpServletResponse resp) {

        Cookie refreshTokenCookie = new Cookie("refreshToken", refresToken);
        refreshTokenCookie.setHttpOnly(true);
        refreshTokenCookie.setSecure(cursoApiProperty.getSeguranca().isEnableHttps());
